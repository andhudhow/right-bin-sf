...
class Game {
    constructor(stage) {
        ...
        createjs.Sound.registerSound("assets/sounds/trap.mp3", "trap");
        createjs.Ticker.addEventListener('tick', stage);
    }

    initialScreen(stage) {
        const intro = new createjs.Text("Welcome: Click to Play", "bold 24px Arial", "#FFFFFF");
        intro.textAlign = "right";
        intro.textBaseline = "middle";
        intro.x = canvas.width / 2;
        intro.y = canvas.height / 2;
    ...
    generateWaste(stage) {
        setInterval(()=>this.createWaste(stage),2500);
    }
