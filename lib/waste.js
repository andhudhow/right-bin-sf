...
class Waste {
    ...
    draw() {
        ...
        if (this.type === 'compost') {
            ...
            waste = new createjs.Bitmap(`./assets/compost_${rand_img_num}.png`);
        } else if (this.type === 'recycle') {
            const rand_img_num = Math.floor(Math.random() * 20);
            waste = new createjs.Bitmap(`./assets/recycle_${rand_img_num}.png`);
        } else if (this.type === 'landfill') {
            const rand_img_num = Math.floor(Math.random() * 10);
            waste = new createjs.Bitmap(`./assets/landfill_${rand_img_num}.png`);
        }
        waste.x = -200;
        waste.scaleX = 0.2;
        waste.scaleY = 0.2;
